...
while iterCrit == 1
    ...
    u = x1_ - x0_;  %reverse displacement
    
    % Utlize predictor information for deformation warping
    if predictor.flag == 1 & cumCount <20
        x0_ = [predictor.x0;x0_];
        x1_ = [predictor.x1;x1_]; 
        u = [predictor.u;u];
        cumCount = cumCount+1;
    end
    ...
    try
        parfor j =1:2
            ...
            F = scatteredInterpolant(x1_(:,1:2),u(:,j),'natural','linear');
            u1(:,j) = -1*F(x1(:,1:2));
        end
        u1(:,3) = 0;
        %         x0 = x0+u0;
    ...
    [sGrid{1},sGrid{2}] = ndgrid(sGrid{1},sGrid{2});
    ...
    for i = 1 : nSSgrid          % Parallel computing
        
        %Subset defining point
        sPt = [sGrid{1}(i),sGrid{2}(i)];
    ...
    % Remove Outliers
    [track] = removeOutlierTPT2(x0,x1,track,outlrThres);
...
while iter<6
    ...
    uPred = zeros(sum(~x0track),2);
    try
        parfor j =1:2
            F = scatteredInterpolant(y0(x0track,1:2),u(:,j),'natural','linear');
            uPred(:,j) = F(y0(~x0track,1:2));
        end
    catch
        disp('scattered interp failed')
    end
    ...
    % Remove Outliers
    [track] = removeOutlierTPT2(y0,y1,track,outlrThres);
    ...
    fprintf('    Elapsed time (Iteration %d): %0.4f secs   Particles tracked: %0.2f  \n',iter,toc(tIter),sum(track>0)/length(x1)*100);
    iter = iter+1;    
end
