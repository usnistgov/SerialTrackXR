function [matches_A2B,u_B2A_curr_refB,track_A2B,matchRatio] = f_track_serial_match2D( parCoordA, parCoordB, varargin )
...
%               u_B2A_curr_refB  - list of particle displacements
%               track_A2B        - list tracked particle matchs
%               matchRatio       - ratio of matched particles to total identified
%
% ---------------------------------------------------
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
smoothness = 1e-4;       % Coefficient of regularization
outlrThres = 8;          % Threshold for removing outliers in TPT (Westerweel et al., Exp.Fluids, 2005)
...
strain_f_o_s = 100;       % Size of virtual strain gauge [px]
strain_n_neighbors = 20; % # of neighboring particles used in strain gauge
BeadparaDistMissing = 30; % Distance threshold to check whether particle has a match or not [px]
...
while iterNum < maxIterNum
    ...
    while isempty(matches_A2B) && n_neighborsMax < length(parNotMissingIndA)
        ...
        if n_neighbors > 12 && n_neighborsMax <= length(parCoordBCurr(parNotMissingIndBCurr,:)) && n_neighborsMax <= length(parCoordA(parNotMissingIndA,:))
            ...
            tptParam.fmThres = 10;
            tptParam.nSpheres = 3;
            tptParam.sizeI = [gridxyROIRange.gridy(2),gridxyROIRange.gridx(2),];
            ...
            if sum(matches_A2B > 0,'all') < (length(parCoordA)/2 + length(matches_A2B))
                ...
                matches_A2B = f_track_nearest_neighbor2( parCoordA(parNotMissingIndA,:), parCoordBCurr(parNotMissingIndBCurr,:), f_o_s );
            end
